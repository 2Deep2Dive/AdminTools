<<<<<<< HEAD:New York PowerShell User Group Sept 2021/Move-OutlookItems.ps1
...
$SenderEMaliAddresses = "pdups1@profidata.com" , "helix@profidata.com"
foreach($SenderEMaliAddress in $SenderEMaliAddresses){
    $MailsToDelete = $InboxItems | Where-Object{ ($_.SenderEmailAddress -like $SenderEMaliAddress) } #| Format-Table SenderEmailAddress, Subject
    $MailsToDelete | ForEach-Object{$_.Delete()}
}
#endregion

#region : Filter based on the email address and attempt to extract the name form the subject
$InboxItems | Where-Object{ $_.SenderEmailAddress -eq $SenderEMaliAddress} | ForEach-Object{
...
$azureFolder = $SubFolders | Where-Object{ $_.Name -eq "*Chocolatey CCM*" }
$InboxItems | Where-Object{ $_.SenderEmailAddress -eq $SenderEMaliAddress } | ForEach-Object{
...
>>>>>>> 272d1c0f9e345e902b9ef3e4d72c8b00c3d873ff:2021-09 New York PowerShell User Group/Move-OutlookItems.ps1
#endregion